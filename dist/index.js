...
function getEditor(editor) {
  ...
  for (const editor2 of allEditors()) {
    ...
    for (const editorPath of editor2.paths) {
      if (path6.normalize(needle) === path6.normalize(editorPath.toLowerCase())) {
        return editor2;
      }
    }
...
function defaultEditor() {
  const editor = process10.env.EDITOR || process10.env.VISUAL;
  if (!editor) {
    throw new Error(
...
var is_wsl_default = process11.env.__IS_WSL_TEST__ ? isWsl : isWsl();
...
var powerShellPath = async () => {
  ...
  return `${process12.env.SYSTEMROOT || process12.env.windir || String.raw`C:\Windows`}\\System32\\WindowsPowerShell\\v1.0\\powershell.exe`;
};
...
// node_modules/open/index.js
var execFile5 = promisify7(childProcess.execFile);
var __dirname = path7.dirname(fileURLToPath3(import.meta.url));
var localXdgOpenPath = path7.join(__dirname, "xdg-open");
var { platform: platform2, arch } = process16;
async function getWindowsDefaultBrowserFromWsl() {
  const powershellPath = await powerShellPath();
  const rawCommand = String.raw`(Get-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\Shell\Associations\UrlAssociations\http\UserChoice").ProgId`;
  const encodedCommand = Buffer5.from(rawCommand, "utf16le").toString("base64");
...
var baseOpen = async (options) => {
  ...
  if (platform2 === "darwin") {
    ...
    if (app) {
      ...
      const useSystemXdgOpen = process16.versions.electron ?? (platform2 === "android" || isBundled || !exeLocalXdgOpen);
...
async function openEditor(files, options = {}) {
  ...
  try {
    ...
    if (result.isTerminalEditor) {
      subprocess.on("exit", process17.exit);
    } else {
      subprocess.unref();
    }
    return true;
