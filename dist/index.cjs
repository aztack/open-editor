...
// node_modules/env-editor/index.js
...
function getEditor(editor) {
  ...
  for (const editor2 of allEditors()) {
    if (needle === editor2.id || needle === editor2.name.toLowerCase() || binary === editor2.binary) {
      return editor2;
    }
    for (const editorPath of editor2.paths) {
      if (import_node_path6.default.normalize(needle) === import_node_path6.default.normalize(editorPath.toLowerCase())) {
        return editor2;
      }
    }
    for (const keyword of editor2.keywords) {
      if (needle === keyword) {
        return editor2;
      }
    }
...
function defaultEditor() {
  const editor = import_node_process11.default.env.EDITOR || import_node_process11.default.env.VISUAL;
  if (!editor) {
    throw new Error(
...
// node_modules/line-column-path/index.js
function parseLineColumnPath(path8) {
  if (typeof path8 === "object") {
    if (!path8.file) {
    ...
    return {
      file: path8.file,
      line: path8.line || 1,
      column: path8.column || 1
    };
  }
  const match = /^(.*?):(\d+)(?::(\d+))?$/.exec(path8);
  if (!match) {
    return {
      file: path8,
      line: 1,
...
function stringifyLineColumnPath(path8, options) {
  options = {
    file: true,
    column: true,
    ...options
  ...
  if (path8.line && path8.column && options.column) {
    result += `:${path8.column}`;
  }
  if (!options.file) {
...
// node_modules/is-wsl/index.js
var isWsl = () => {
  if (import_node_process12.default.platform !== "linux") {
    return false;
  }
...
var is_wsl_default = import_node_process12.default.env.__IS_WSL_TEST__ ? isWsl : isWsl();
...
var wslDrivesMountPoint = /* @__PURE__ */ (() => {
  ...
  return async function() {
    ...
    try {
      await import_promises12.default.access(configFilePath, import_promises12.constants.F_OK);
    ...
    const configContent = await import_promises12.default.readFile(configFilePath, { encoding: "utf8" });
...
var powerShellPath = async () => {
  ...
  return `${import_node_process13.default.env.SYSTEMROOT || import_node_process13.default.env.windir || String.raw`C:\Windows`}\\System32\\WindowsPowerShell\\v1.0\\powershell.exe`;
};
...
var execFileAsync = (0, import_node_util12.promisify)(import_node_child_process6.execFile);
...
var execFileAsync2 = (0, import_node_util13.promisify)(import_node_child_process7.execFile);
async function runAppleScript(script, { humanReadableOutput = true } = {}) {
  if (import_node_process15.default.platform !== "darwin") {
    throw new Error("macOS only");
  }
...
var execFileAsync3 = (0, import_node_util14.promisify)(import_node_child_process8.execFile);
...
// node_modules/default-browser/index.js
var execFileAsync4 = (0, import_node_util15.promisify)(import_node_child_process9.execFile);
var titleize = (string) => string.toLowerCase().replaceAll(/(?:^|\s|-)\S/g, (x) => x.toUpperCase());
async function defaultBrowser2() {
  if (import_node_process16.default.platform === "darwin") {
    const id = await defaultBrowserId();
    const name = await bundleName(id);
    return { name, id };
  }
  if (import_node_process16.default.platform === "linux") {
    const { stdout } = await execFileAsync4("xdg-mime", ["query", "default", "x-scheme-handler/http"]);
    const id = stdout.trim();
    const name = titleize(id.replace(/.desktop$/, "").replace("-", " "));
    return { name, id };
  }
  if (import_node_process16.default.platform === "win32") {
...
// node_modules/open/index.js
var execFile5 = (0, import_node_util16.promisify)(import_node_child_process10.default.execFile);
var __dirname = import_node_path7.default.dirname((0, import_node_url3.fileURLToPath)(`file:///${__filename}`));
var localXdgOpenPath = import_node_path7.default.join(__dirname, "xdg-open");
var { platform: platform2, arch } = import_node_process17.default;
async function getWindowsDefaultBrowserFromWsl() {
  const powershellPath = await powerShellPath();
  const rawCommand = String.raw`(Get-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\Shell\Associations\UrlAssociations\http\UserChoice").ProgId`;
  const encodedCommand = import_node_buffer4.Buffer.from(rawCommand, "utf16le").toString("base64");
...
var baseOpen = async (options) => {
  ...
  if (platform2 === "darwin") {
    ...
    options.target = import_node_buffer4.Buffer.from(encodedArguments.join(" "), "utf16le").toString("base64");
  } else {
    if (app) {
      command = app;
    } else {
      ...
      try {
        await import_promises13.default.access(localXdgOpenPath, import_promises13.constants.X_OK);
        exeLocalXdgOpen = true;
      } catch {
      }
      const useSystemXdgOpen = import_node_process17.default.versions.electron ?? (platform2 === "android" || isBundled || !exeLocalXdgOpen);
  ...
  const subprocess = import_node_child_process10.default.spawn(command, cliArguments, childProcessOptions);
  if (options.wait) {
...
async function openEditor(files, options = {}) {
  ...
  try {
      ...
      subprocess.on("error", async (error) => {
        if (!resolved) {
          ...
          if (fallback) {
            for (const file of files) {
              const parsed = parseLineColumnPath(file);
              await open_default(parsed.file);
            }
            resolve(true);
          } else {
            reject(error);
    ...
    if (result.isTerminalEditor) {
      subprocess.on("exit", import_process.default.exit);
    } else {
      subprocess.unref();
    }
    return true;
